...
def fetch_deriv_price_and_candles():
    """
    Mengambil harga live tick dan data historis/klines secara murni dari 
    Public Deriv WebSocket API (wss://ws.derivws.com/websockets/v3?app_id=1089)
    tanpa menggunakan Binance sama sekali.
    """
    ...
    try:
        ...
        # 1. Request live tick price
        ws.send(json.dumps({"ticks": SYMBOL}))
        
        # 2. Request historical candles (M15)
        req_candles = {
        ...
        ws.send(json.dumps(req_candles))
        
        # Loop to capture responses
        for _ in range(10):
            res_str = ws.recv()
            if not res_str:
                continue
            ...
            if res.get("msg_type") == "tick" and res.get("tick", {}).get("symbol") == SYMBOL:
                live_price = float(res["tick"]["quote"])
                
            elif res.get("msg_type") == "candles":
                candles = res.get("candles", [])
                
            if live_price != 4305.00 and len(candles) > 0:
                break
                
        ws.close()
    except Exception as e:
        log(f"Deriv WS connection err: {e}")

    # Fallback jika candles kosong
    if not candles:
        raise RuntimeError("Gagal menarik data candles dari Public Deriv WebSocket.")
        
    df = pd.DataFrame(candles)
    # Kolom deriv candles: epoch, open, high, low, close
    df["close"] = df["close"].astype(float)
    df["high"] = df["high"].astype(float)
    df["low"] = df["low"].astype(float)
    df["open"] = df["open"].astype(float)
    df["volume"] = 100.0  # Default volume weight untuk deriv feed
...
def main():
    ...
    try:
        ...
        # Ambil harga real-time dan klines murni dari WebSocket publik Deriv
        price, df_m15 = fetch_deriv_price_and_candles()

#!/usr/bin/env python3
# ULTIMATE OMNI-BOT V21.3 - DERIV WEBSOCKET SYNCED (frxXAUUSD)
...
def fetch_price():
    # Mengambil harga real-time presisi langsung dari WebSocket Publik Deriv
...
def make_chart(df, entry, sl, t1, t2, t3, t4, signal, price, sama, golden_label, sl_dyn, tp3_dyn):
    try:
        ...
        ax1.plot(df_last["time"], df_last["close"], color='#FFD700', linewidth=2, label=f'frxXAUUSD Deriv Feed {price:.2f}')
...
def main():
    ...
    log("🔥 ULTIMATE OMNI-BOT V21.3 DERIV WEBSOCKET START")
    try:

#!/usr/bin/env python3
# ULTIMATE OMNI-BOT V21.3 - FULL PERFECTED & MT5 SYNCED
...
SYMBOL = "PAXGUSDT"  
QUORUM_PERCENT = 65.0  
PRICE_OFFSET = -14.0  # Penyesuaian presisi agar klop 100% dengan MT5 (4286)
...
def fetch_price():
    ...
    for u in urls:
        try:
            r = requests.get(u, headers=random.choice(HDRS), timeout=5)
            if r.ok:
                raw_p = float(r.json()["price"])
                return raw_p + PRICE_OFFSET
        except:
            continue
    return 4286.00
...
def make_chart(df, entry, sl, t1, t2, t3, t4, signal, price, sama, golden_label, sl_dyn, tp3_dyn):
    try:
        ...
        df_last["ema9"] = df_last["close"].ewm(9).mean() + PRICE_OFFSET
        df_last["ema21"] = df_last["close"].ewm(21).mean() + PRICE_OFFSET
        df_last["close_sync"] = df_last["close"] + PRICE_OFFSET
        df_last["high_sync"] = df_last["high"] + PRICE_OFFSET
        df_last["low_sync"] = df_last["low"] + PRICE_OFFSET
        
        tp = (df_last["high_sync"] + df_last["low_sync"] + df_last["close_sync"]) / 3
        ...
        ax1.plot(df_last["time"], df_last["close_sync"], color='#FFD700', linewidth=2, label=f'XAUUSD MT5 Sync {price:.2f}')
        ...
        colors = ['#00FF88' if c >= o else '#FF3B3B' for c, o in zip(df_last["close_sync"], df_last["open"] + PRICE_OFFSET)]
...
def main():
    ...
    log("🔥 ULTIMATE OMNI-BOT V21.3 PERFECTED START")
    try:
        ...
        if signal != "NEUTRAL":
            ...
            caption = (
                ...
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🔹 <b>Entry (MT5 Synced):</b> {entry:.2f}\n"

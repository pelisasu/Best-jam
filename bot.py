#!/usr/bin/env python3
# ULTIMATE OMNI-BOT V21.3 - DERIV HTTP REST SYNCED (frxXAUUSD)
...
# File Penyimpanan (Atomic Write Safe)
STATE_FILE = "bot_state.json"
JOURNAL_FILE = "trade_journal.json"
DNA_FILE = "dna_15_engines.json"

SYMBOL = "frxXAUUSD"  # Simbol resmi Deriv untuk MT5 Gold
QUORUM_PERCENT = 65.0  
...
def fetch_klines(interval, limit):
    # Mengambil data historis klines dari Binance sebagai basis kalkulasi engine indikator yang stabil
    ...
    urls = [
        ...
        f"https://data-api.binance.vision/api/v3/klines?symbol={binance_symbol}&interval={interval}&limit={limit}",
    ]
    for u in urls:
        for _ in range(3):
            try:
                r = requests.get(u, headers=random.choice(HDRS), timeout=8)
...
def fetch_price():
    # Mengambil harga live akurat langsung dari endpoint publik Deriv API (HTTP REST v3)
    try:
        url = "https://green.derivws.com/websockets/v3?app_id=1089&passthrough_oc=1"
        # Menggunakan format query HTTP open API Deriv
        r = requests.get(f"https://api.deriv.com/v1/ticker?symbol={SYMBOL}", timeout=5)
        if r.ok:
            data = r.json()
            if "price" in data:
    ...
    # Fallback alternatif menggunakan PAXG Binance jika endpoint HTTP Deriv sedang sibuk
    try:
...
def make_chart(df, entry, sl, t1, t2, t3, t4, signal, price, sama, golden_label, sl_dyn, tp3_dyn):
    try:
        ...
        ax1.plot(df_last["time"], df_last["close"], color='#FFD700', linewidth=2, label=f'frxXAUUSD Feed {price:.2f}')
...
def main():
    ...
    log("🔥 ULTIMATE OMNI-BOT V21.3 DERIV HTTP START")
    try:
